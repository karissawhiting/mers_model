library(tmvtnorm)
mcmcMH2<- function (target, init.theta, proposal.sd = NULL, n.iterations, 
          covmat = NULL, limits = list(lower = NULL, upper = NULL), 
          adapt.size.start = NULL, adapt.size.cooling = 0.99, adapt.shape.start = NULL, 
          adapt.shape.stop = NULL, print.info.every = n.iterations/100, 
          verbose = FALSE, max.scaling.sd = 50) 
{
  ...
  for (i.iteration in seq_len(n.iterations)) {
    ...
    else if (!is.null(adapt.shape.start) && acceptance.rate * i.iteration >= adapt.shape.start && (adapting.shape == 0 || i.iteration < adapting.shape + adapt.shape.stop)) {
      if (!adapting.shape) {
    ...
    if (i.iteration%%ceiling(print.info.every) == 0) {
      ...
      message(Sys.time(), ", Iteration: ", i.iteration, 
              "/", n.iterations, ", acceptance rate: ", sprintf("%.3f", 
                                                                acceptance.rate), appendLF = FALSE)
